...
## In-development exact-profile lane
...
| --- | ---: | --- |
| State-only baked geometry | 206 | Route to the exported-geometry renderer |
| Block-entity renderer | 8 | Original-resource fallback |
| Adjustable model data | 4 | Original-resource fallback |
| Collapsible model data | 2 | Original-resource fallback |
| Flower model data | 1 | Original-resource fallback |
| One-way-window model data | 1 | Original-resource fallback |
| Special camouflage overlay | 12 | Original-resource fallback |
...
the 28 fallback IDs, removing 4,038 quads that can never enter the add-on's
positive route. The retained geometry uses 18 fixed sprite identifiers, all in
the `minecraft` or `framedblocks` namespace. Empty fallback templates remain
in the inventory so policy and exact-state validation cannot silently broaden
coverage.

Unknown IDs never become supported implicitly. Routed IDs also fall back for
waterlogged, dynamic-light/skylight, reinforced, or framed-adjacent contexts.
The last rule is necessary because FramedBlocks hidden-face model data depends
on every neighboring framed block, not only on the block being rendered.
...
light inputs. Its bounded expanded runtime fixtures have passed; the remaining
visual, malformed-profile, concurrency, and performance limits below still
constrain the support claim.
...
## Observed expanded exact-profile fixtures
...
The isolated full-pack gallery constructed and server-verified one generated
default-state anchor for each of the 234 placeable/displayable IDs. Its
...
- human-approved and pixel-repeatable modded-client reference;
